...
void GeneticAlgorithm<T, conf>::Initialize() {
  ...
  pSelectionObject = pSelectionObjectFactory->CreateSelectionObject(
      static_cast<SelectionMechanism>(conf.selectionMechanism));
}
...
template <typename T, Config conf> void GeneticAlgorithm<T, conf>::Run() {
  ...
  for (; generationNumber < nNumberOfGenerations; generationNumber++) {
    spdlog::info("Generation {0} Best Fitness {1}", generationNumber,
...
template <typename T, Config conf> void GeneticAlgorithm<T, conf>::Evolve() {
  ...
  for (std::size_t i = elitismOffset; i < conf.populationSize; i++) {
    ...
    auto breededIndividual =
        Crossover(bestFirstIndividual, bestSecondIndividual);
...
template <typename T, Config conf>
Chromosome<T, conf>
GeneticAlgorithm<T, conf>::Crossover(const Chromosome<T, conf> &x,
                                     const Chromosome<T, conf> &y) const {
  ...
  for (std::size_t i = 0; i < conf.geneLength; i++) {
    ...
    if (definitelyLessThan<double>(crossoverProbability, conf.crossoverRate,
...
template <typename T, Config conf> void GeneticAlgorithm<T, conf>::Mutate() {

  for (std::size_t i = 0; i < conf.populationSize; i++) {
    double mutationProbability = uniform<double>(0, 1);
    if (definitelyLessThan<double>(mutationProbability,
